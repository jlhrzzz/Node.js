...
class ReadStream extends EventEmitter {
  ...
  read () { // once events模块中的绑定一次
    ...
    let howMuchRead = this.end ? Math.min(this.end - this.offset + 1, this.highWaterMark) : this.highWaterMark

    const buffer = Buffer.alloc(howMuchRead)
    // 读取文件中内容，每次读取this.highWaterMark个
    // 123 4
    fs.read(this.fd, buffer, 0, howMuchRead, this.offset, (err, bytesRead) => {
      if (bytesRead) {
